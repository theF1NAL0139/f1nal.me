...
const PlayPage = ({ onOpenImage }: { onOpenImage: (src: string) => void }) => {
  ...
  useEffect(() => {
    ...
    const loadMedia = async () => {
        const MAX_CHECK = 30; // Check up to 30 items
        const promises = [];
        const items: MediaItem[] = [];

        for (let i = 1; i <= MAX_CHECK; i++) {
            // 1. Check existing images: imgs/img_i.jpg
            promises.push(
                checkImage(`imgs/img_${i}.jpg`).then(exists => {
                     if (exists && isMounted) items.push({ id: i, src: `imgs/img_${i}.jpg`, type: 'image' });
                })
            );
            
            // 2. Check animations: anim/anim_i.gif
            promises.push(
                checkImage(`anim/anim_${i}.gif`).then(exists => {
                     if (exists && isMounted) items.push({ id: i + 1000, src: `anim/anim_${i}.gif`, type: 'image' });
                })
            );
            
            // 3. Check videos: anim/anim_i.mp4
            promises.push(
                 checkVideo(`anim/anim_${i}.mp4`).then(exists => {
                     if (exists && isMounted) items.push({ id: i + 2000, src: `anim/anim_${i}.mp4`, type: 'video' });
                 })
            );
        }
        ...
        if (isMounted) {
            // Sort by number in filename for consistency (mixes img_1, anim_1, img_2, etc.)
            items.sort((a, b) => {
